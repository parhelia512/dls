module dcd.server.dscanner.analysis.mismatched_args;

import dcd.server.dscanner.analysis.base;
import dcd.server.dscanner.utils : safeAccess;
...
final class MismatchedArgumentCheck : BaseAnalyzer
{
	...
	override void visit(const FunctionCallExpression fce)
	{
		...
		foreach (sym; symbols)
		{
			...
			// resolution code.
			const(istring)[] params = sym is null ? [] : sym.functionParameters[].map!(a => cast() a.name).array();
